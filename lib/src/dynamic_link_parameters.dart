...
part of firebase_dynamic_links_fixed;
...
class DynamicLinkParameters {
  ...
  static Future<ShortDynamicLink> shortenUrl(
    Uri url, [
    DynamicLinkParametersOptions? options,
  ]) async {
    final Map<dynamic, dynamic> reply =
        await FirebaseDynamicLinks.channel.invokeMethod(
  ...
  Future<Uri> buildUrl() async {
    final String url = await FirebaseDynamicLinks.channel
        .invokeMethod<String>('DynamicLinkParameters#buildUrl', _data) as String;
  ...
  static ShortDynamicLink _parseShortLink(Map<dynamic, dynamic> reply) {
    final List<dynamic>? warnings = reply['warnings'] as List<dynamic>?;
    return ShortDynamicLink._(
      Uri.parse(reply['url'] as String),
      warnings?.cast<String>(),
    );
  }
}
